...
class SendEmail
{
     ...
     */
    public function send_email_verification($data, $case=null, $doctorEnc=null)
    {
        $message = '';
        ...
        //$body = str_replace('{{user_name}}', $data['first_name'] ? ucfirst(libsodiumDecrypt($data['first_name']) . ' ' . libsodiumDecrypt($data['last_name'])) : "", $body);
        $body = str_replace('{{website_name}}', !empty(settings("website_name")) ? settings("website_name") : "Textteo", $body);
        $body = !$case && $case == 'QLS28lY0bAu914UOBiXZZFZ5HH80AgiO' &&  $doctorEnc != null?
                str_replace('{{verify_url}}', base_url() . 'doctor-preview/' . $doctorEnc.'?keyword='.md5($data['id']).'&keyword-case='.$case, $body):
                str_replace('{{verify_url}}', base_url() . 'activate/' . md5($data['id']), $body);
